...
public class CircularProgressBarRed extends ProgressBar{
    ...
    CircularProgressBarRed c1 = (CircularProgressBarRed) findViewById(R.id.circularprogressbarred);
    ...
    public interface ProgressAnimationListenerRed{
        public void onAnimationStart();
        public void onAnimationFinish();
        public void onAnimationProgress(int progress);
    }

    public CircularProgressBarRed(Context context) {
        super(context);
        init(null, 0);
    }

    public CircularProgressBarRed(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(attrs, 0);
    }

    public CircularProgressBarRed(Context context, AttributeSet attrs, int defStyle) {
    ...
    public void init(AttributeSet attrs, int style){
        ...
        if(color==null)
            mProgressColorPaint.setColor(res.getColor(R.color.circular_progress_default_progress_red));
        else
            mProgressColorPaint.setColor(Color.parseColor(color));

        color = a.getString(R.styleable.CircularProgressBar_backgroundColor);
        if(color==null)
            mBackgroundColorPaint.setColor(res.getColor(R.color.circular_progress_default_background_red));
        else
            mBackgroundColorPaint.setColor(Color.parseColor(color));

        color = a.getString(R.styleable.CircularProgressBar_titleColor);
        if(color==null)
            mTitlePaint.setColor(res.getColor(R.color.circular_progress_default_title_red));
        else
            mTitlePaint.setColor(Color.parseColor(color));

        color = a.getString(R.styleable.CircularProgressBar_subtitleColor);
        if(color==null)
            mSubtitlePaint.setColor(res.getColor(R.color.circular_progress_default_subtitle_red));
        else
    ...
    public void animateProgressTo(final int start, final int end, final ProgressAnimationListenerRed listener){
        if(start!=0)
            setProgress(start);
        ...
        progressBarAnimator.addListener(new AnimatorListener() {
            ...
            public void onAnimationEnd(final Animator animation) {
                CircularProgressBarRed.this.setProgress(end);
                if(listener!=null)
        ...
        progressBarAnimator.addUpdateListener(new AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(final ValueAnimator animation) {
                ...
                if(progress!=CircularProgressBarRed.this.getProgress()){
                    Log.d(TAG, progress + "");
                    CircularProgressBarRed.this.setProgress(progress);
                    if(listener!=null)
