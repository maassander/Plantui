...
public class CircularProgressBarGreen extends ProgressBar{
    ...
    CircularProgressBarGreen c2 = (CircularProgressBarGreen) findViewById(R.id.circularprogressbargreen);
    ...
    public boolean   onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                a=getMax() - (int) (getMax() * event.getY() / getHeight());
                setProgress(a);
    ...
    public interface ProgressAnimationListenerGreen{
        public void onAnimationStart();
        public void onAnimationFinish();
        public void onAnimationProgress(int progress);
    }

    public CircularProgressBarGreen(Context context) {
        super(context);
        init(null, 0);
    }

    public CircularProgressBarGreen(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(attrs, 0);
    }

    public CircularProgressBarGreen(Context context, AttributeSet attrs, int defStyle) {
    ...
    public void init(AttributeSet attrs, int style){
        ...
        if(color==null)
            mProgressColorPaint.setColor(res.getColor(R.color.circular_progress_default_progress_green));
        else
            mProgressColorPaint.setColor(Color.parseColor(color));

        color = a.getString(R.styleable.CircularProgressBar_backgroundColor);
        if(color==null)
            mBackgroundColorPaint.setColor(res.getColor(R.color.circular_progress_default_background_green));
        else
            mBackgroundColorPaint.setColor(Color.parseColor(color));

        color = a.getString(R.styleable.CircularProgressBar_titleColor);
        if(color==null)
            mTitlePaint.setColor(res.getColor(R.color.circular_progress_default_title_green));
        else
            mTitlePaint.setColor(Color.parseColor(color));

        color = a.getString(R.styleable.CircularProgressBar_subtitleColor);
        if(color==null)
            mSubtitlePaint.setColor(res.getColor(R.color.circular_progress_default_subtitle_green));
        else
            mSubtitlePaint.setColor(Color.parseColor(color));


        String t = a.getString(R.styleable.CircularProgressBar_titles);
        if(t!=null)
            mTitle = t;

        t = a.getString(R.styleable.CircularProgressBar_subtitles);
        if(t!=null)
            mSubTitle = t;
    ...
    public void animateProgressTo(final int start, final int end, final ProgressAnimationListenerGreen listener){
        if(start!=0)
            setProgress(start);
        ...
        progressBarAnimator.addListener(new AnimatorListener() {
            ...
            public void onAnimationEnd(final Animator animation) {
                CircularProgressBarGreen.this.setProgress(end);
                if(listener!=null)
        ...
        progressBarAnimator.addUpdateListener(new AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(final ValueAnimator animation) {
                ...
                if(progress!=CircularProgressBarGreen.this.getProgress()){
                    Log.d(TAG, progress + "");
                    CircularProgressBarGreen.this.setProgress(progress);
                    if(listener!=null)
