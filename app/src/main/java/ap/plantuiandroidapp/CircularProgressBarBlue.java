...
public class CircularProgressBarBlue extends ProgressBar{
    ...
    CircularProgressBarBlue c3 = (CircularProgressBarBlue) findViewById(R.id.circularprogressbarblue);
    ...
    public boolean   onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                int b=0;
                b=getMax() - (int) (getMax() * event.getY() / getHeight());
                setProgress(b);
    ...
    public interface ProgressAnimationListenerBlue{
        public void onAnimationStart();
        public void onAnimationFinish();
        public void onAnimationProgress(int progress);
    }

    public CircularProgressBarBlue(Context context) {
        super(context);
        init(null, 0);
    }

    public CircularProgressBarBlue(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(attrs, 0);
    }

    public CircularProgressBarBlue(Context context, AttributeSet attrs, int defStyle) {
    ...
    public void init(AttributeSet attrs, int style){
        ...
        if(color==null)
            mProgressColorPaint.setColor(res.getColor(R.color.circular_progress_default_progress_blue));
        else
            mProgressColorPaint.setColor(Color.parseColor(color));

        color = a.getString(R.styleable.CircularProgressBar_backgroundColor);
        if(color==null)
            mBackgroundColorPaint.setColor(res.getColor(R.color.circular_progress_default_background_blue));
        else
            mBackgroundColorPaint.setColor(Color.parseColor(color));

        color = a.getString(R.styleable.CircularProgressBar_titleColor);
        if(color==null)
            mTitlePaint.setColor(res.getColor(R.color.circular_progress_default_title_blue));
        else
            mTitlePaint.setColor(Color.parseColor(color));

        color = a.getString(R.styleable.CircularProgressBar_subtitleColor);
        if(color==null)
            mSubtitlePaint.setColor(res.getColor(R.color.circular_progress_default_subtitle_blue));
        else
    ...
    public void animateProgressTo(final int start, final int end, final ProgressAnimationListenerBlue listener){
        if(start!=0)
            setProgress(start);
        ...
        progressBarAnimator.addListener(new AnimatorListener() {
            ...
            public void onAnimationEnd(final Animator animation) {
                CircularProgressBarBlue.this.setProgress(end);
                if(listener!=null)
        ...
        progressBarAnimator.addUpdateListener(new AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(final ValueAnimator animation) {
                ...
                if(progress!=CircularProgressBarBlue.this.getProgress()){
                    Log.d(TAG, progress + "");
                    CircularProgressBarBlue.this.setProgress(progress);
                    if(listener!=null)
